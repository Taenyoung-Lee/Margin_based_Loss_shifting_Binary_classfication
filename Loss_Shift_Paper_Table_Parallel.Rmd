...
simulate_once <- function(n_train, n_test, rate_pos, rate_neg, seed = NULL) {
  ...
  for (st in c("none","soft","hard")) for (bl in c("hinge","sqhinge","logistic","exp")) {
    ...
    res <- train_loss_shift(
      ...
      max_iter = 100, line_search = TRUE, verbose = TRUE,
...
## ───────────────────────────────────────────────────────────────────
## 8) 병렬 실행을 위한 그리드 실행 + 표 생성
##    (Dependencies: future, future.apply)
## ───────────────────────────────────────────────────────────────────
suppressPackageStartupMessages({
  library(future)
...
run_grid_parallel <- function(n_vec, rates_pos, rates_neg, n_test, k = 20, seed = 2025, digits = 3) {
  p <- 2

  # 1. Setup Parallel Backend
  # -------------------------
  # 'multisession' works on all OS (Windows, macOS, Linux).
  # It creates separate R sessions for each worker.
  # By default, it uses all available cores minus one.
  cat(sprintf("Setting up parallel backend using %d workers...\n", availableCores()))
  plan(multisession)

  # 2. Create Parameter Grid
  # ------------------------
  # This creates a data frame with every combination of the input parameters.
  param_grid <- expand.grid(n = n_vec, rp = rates_pos, rn = rates_neg,
                            stringsAsFactors = FALSE)

  # 3. Run Simulations in Parallel
  # ------------------------------
  # future_lapply is a parallel version of lapply. It iterates through
  # each row of our parameter grid and executes the code.
  #
  # 'future.seed = TRUE' is CRITICAL for reproducible random results
  # in parallel. It ensures each worker gets a unique, valid random seed.
  cat(sprintf("Running %d simulation sets in parallel (k=%d reps each)...\n", nrow(param_grid), k))
  
  # Use progressr to show a progress bar (optional, but recommended for long jobs)
  progressr::with_progress({
    p_bar <- progressr::progressor(steps = nrow(param_grid))
    
    rows_list <- future_lapply(1:nrow(param_grid), function(i) {
      # Get parameters for the current run from the grid
      n_run <- param_grid$n[i]
      rp_run <- param_grid$rp[i]
      rn_run <- param_grid$rn[i]

      # Run the original k-repetition simulation function
      R <- simulate_k(
      ...
      # Return the formatted row of results
  ...
  df <- df[ , c("n","p","rate_pos","rate_neg","true", MODEL_COLS)]
  # Sort the final data frame for consistency
  df[order(df$n, df$p, df$rate_pos, df$rate_neg), ]
}
...
# Just call the new parallel function instead of the old one
tbl_parallel <- run_grid_parallel(
  n_vec     = c(200),
...
write.csv(tbl_parallel, "0814_asymflip_table_parallel.csv", row.names = FALSE)
