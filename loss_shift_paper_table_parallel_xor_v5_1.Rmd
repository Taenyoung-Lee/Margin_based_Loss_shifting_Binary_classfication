...
pred_e1071_tuned <- function(Xtr, ytr, Xte, kernel = "radial", seed = NULL) {
  y_fac <- factor(ifelse(ytr > 0, "pos", "neg"))

  if (kernel != "radial") { # Fallback for linear kernel
    fit <- e1071::svm(x = Xtr, y = y_fac, kernel = "linear", type = "C-classification", cost = 1.0, scale = FALSE)
  } else {
    # Define hyperparameter grids consistent with loss_shifting
    # sigma -> gamma conversion: gamma = 1 / (2 * sigma^2)
    sigma_grid <- .sigma_grid_from_median_scaled(Xtr, sigma_mult = c(0.5, 1, 2), seed = seed)
    gamma_grid <- 1 / (2 * sigma_grid^2)
    cost_grid <- 2^seq(-2, 5, len = 6)

    # Use tune() to perform 3-fold CV
    if (!is.null(seed)) set.seed(seed)
    tuned <- e1071::tune(
      svm,
...
run_single_model <- function(model_name, Xtr, ytr, Xte, kernel) {
  switch(model_name,
    "e1071" = pred_e1071_tuned(Xtr, ytr, Xte, kernel = kernel), # Call tuned e1071
