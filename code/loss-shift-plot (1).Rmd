...
source("loss_shifting_v2.r")  # train_loss_shift(), predict_loss_shift(), ...
                                    ...
                                    flip_frac_neg = 0.1)  # label_flip -1 클래스용
{
  ...
  pos   <- matrix(rnorm(2 * n_pos, 1, 1), ncol = 2)
  neg   <- matrix(rnorm(2 * n_neg, -1, 1), ncol = 2)
  ...
  for (i in seq_len(nrow(combos))) {
    ...
    results[[tag]] <- tryCatch({
      mod_wrap <- train_loss_shift(
        ...
        eta_grid = c(0.5, 1, 2), n_folds = 3, restarts = 5, verbose = FALSE)
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                         flip_frac_pos = 0.0, flip_frac_neg = 0.0)
```
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                        flip_frac_pos = 0.05, flip_frac_neg = 0.00)
```
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                        flip_frac_pos = 0.1, flip_frac_neg = 0.00)
```
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                        flip_frac_pos = 0.15, flip_frac_neg = 0.00)
```
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                        flip_frac_pos = 0.05, flip_frac_neg = 0.05)
```
...
run_and_visualize_final(seed = 42, noise_type = "label_flip", 
                        flip_frac_pos = 0.1, flip_frac_neg = 0.05)
```
### Case 3: 비대칭적인 라벨 뒤집기 강화 (Reinforce Asymmetric Label Flipping)
