...
make_two_moon <- function(n,
  ...
  if (mode == "outlier") {
    ...
    if (k > 0) {
      ...
      X[neg_idx[seq_len(k)], ] <- donor + matrix(rnorm(2 * k, 0, noise), nrow = k)
    }
  }
...
run_benchmark <- function(
  ...
  ## 3-1 데이터 생성 ------------------------------------------------------
  raw <- do.call(make_two_moon, c(list(n = n,
                                       mode = data_mode),
                                  make_two_moon_arg))
  ...
  ########################################################################
  # D) GLMNET (선형 / RFF) -----------------------------------------------
  glmnet_out <- foreach(tag = c("GLMNet_rbf"),  #"GLMNet_linear",추가가능
                        .combine = bind_rows,
