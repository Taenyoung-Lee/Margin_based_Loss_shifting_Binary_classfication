...
```{r}
# ───────────────────────────────────────────────────────────────────
# Loss-shifting 실험 (iteration별 즉시 저장 + p∈{2,5,10} + svm_dual=T/F 동시)
# 데이터 분포: Uniform[-a, a]^p + product-sign 라벨 (0이면 rnorm 기준 ±1)
# 파일 구성: 이 스크립트 + 동일 폴더의 loss_shifting_v4.R
# ───────────────────────────────────────────────────────────────────
...
# ───────────────────────────────────────────────────────────────────
build_model_names <- function(include_dual = TRUE) {
...
run_single_model <- function(model_name, Xtr_raw, ytr, Xte_raw, Xtr_s, Xte_s, kernel, seed = NULL) {
  ...
  res <- train_loss_shift(
    ...
    restarts = 10, n_folds = 3, svm_dual = svm_dual,
...
run_grid_fully_parallel <- function(n_train, n_test,
  ...
  model_names <- build_model_names(include_dual = TRUE)

  cat(sprintf("\n--- Running %d jobs in parallel (p ∈ {%s}, k=%d) ---\n",
              nrow(task_grid), paste(p_values, collapse = ","), k))
...
N_TRAIN <- 100
N_TEST  <- 400
P_VALUES <- c(2, 5)              # 필요 시 c(2,5,10) 등으로 확장
RATES_POS <- c(0.00, 0.05, 0.10)
RATES_NEG <- c(0.00, 0.05)
K_REPS <- 3
SEED   <- 25
RESULTS_DIR <- "results_incremental"
...
# ───────────────────────────────────────────────────────────────────
# 6) (옵션) 중간에 중단되었을 때, 폴더에서 재집계하는 헬퍼
# ───────────────────────────────────────────────────────────────────
reaggregate_from_dir <- function(results_dir = "results_incremental",
                                 out_full = "xor_full_iteration_log.csv",
                                 out_agg  = "xor_agg_summary.csv") {
  fs <- list.files(results_dir, pattern = "^res_.*\\.csv$", full.names = TRUE)
  if (!length(fs)) {
    message("No CSV found in ", results_dir)
    return(invisible(NULL))
  }
  lst <- lapply(fs, utils::read.csv)
  full <- bind_rows(lst)
  utils::write.csv(full, out_full, row.names = FALSE)

  agg <- full %>%
    group_by(p, rate_pos, rate_neg, model) %>%
    summarise(mean_acc = mean(accuracy), sd_acc = sd(accuracy), .groups = "drop")
  utils::write.csv(agg, out_agg, row.names = FALSE)

  message("Reaggregated: ", out_full, " / ", out_agg)
  invisible(list(full = full, agg = agg))
}
